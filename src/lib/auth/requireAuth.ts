...
export function requireAuth<T extends Record<string, string> = Record<string, string>>(
  ...
  return async (req: AuthenticatedRequest, context: { params: Promise<T> }) => {
    try {
      ...
      // 3. For state-changing methods, require CSRF token
      const method = req.method?.toUpperCase?.()
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method || "")) {
        const csrfHeader = req.headers.get("x-csrf-token")
        const csrfCookie = maybeNextReq.cookies.get("csrf_token")?.value
      ...
      // 4. Convert string id → ObjectId for DB lookup
